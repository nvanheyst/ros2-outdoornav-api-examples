...
| `parameter_runtime.py` | `<node>/list_parameters`, `<node>/get_parameters`, `<node>/set_parameters` | List, get, and set parameters on any running node at runtime — without restarting it. Useful for tuning controller speed limits or collision-monitor thresholds live. |
